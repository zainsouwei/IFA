...
# import multiprocessing
# from functools import partial
# from preprocessing import load_subject #TODO switch back to HCP
# from sklearn.linear_model import ElasticNet
# from sklearn.model_selection import train_test_split
# from sklearn.metrics import r2_score
...
#     A = ((Xn-Xn.mean(axis=1,keepdims=True)) @ np.linalg.pinv(z_maps_demeaned.T))  # A is Time x Components (T x C)
...
#         Xn.flags.writeable = False  # Mark the array as read-only.        
#         # Calculate for IFA z_maps
#         IFA_An, IFA_spatial_map, IFA_reconstruction_error, IFA_Adm, IFA_spatial_mapdm = calculate_netmat_and_spatial_map(Xn, IFA_z_maps)
        
#         # Calculate for ICA z_maps
#         ICA_An, ICA_spatial_map, ICA_reconstruction_error, ICA_Adm, ICA_spatial_mapdm = calculate_netmat_and_spatial_map(Xn, ICA_z_maps)
        
#         return (IFA_An, IFA_spatial_map, IFA_reconstruction_error, IFA_Adm, IFA_spatial_mapdm), \
#                (ICA_An, ICA_spatial_map, ICA_reconstruction_error, ICA_Adm, ICA_spatial_mapdm)
...
#     with ProcessPoolExecutor(max_workers=int(20)) as executor:
...
#         IFA_An_all, IFA_spatial_map_all, IFA_reconstruction_error_all, \
#         IFA_Adm_all, IFA_spatial_mapdm_all = zip(*[(res[0][0], res[0][1], res[0][2], res[0][3], res[0][4]) 
#                                                                      for res in results if res[0] is not None])

#         ICA_An_all, ICA_spatial_map_all, ICA_reconstruction_error_all, \
#         ICA_Adm_all, ICA_spatial_mapdm_all = zip(*[(res[1][0], res[1][1], res[1][2], res[1][3], res[1][4]) 
#                                                                      for res in results if res[1] is not None])
#         # Return both normalized and demeaned results for further use if needed
#         return ((np.array(IFA_An_all), np.array(IFA_spatial_map_all), np.array(IFA_reconstruction_error_all)), (np.array(IFA_Adm_all), np.array(IFA_spatial_mapdm_all))),((np.array(ICA_An_all), np.array(ICA_spatial_map_all), np.array(ICA_reconstruction_error_all)),(np.array(ICA_Adm_all), np.array(ICA_spatial_mapdm_all)))