...
def voxelwise_FKT_partial(groupA=None, groupB=None, vt=None, n_filters_per_group=1, groupA_paths=None, groupB_paths=None, paths=False,log=False,shrinkage=0.01,cov_method='svd',outputfolder='Path', save=False):
    try:
        assert (not log) or (cov_method == 'svd'), "If log is True, then method must be 'svd'."
        if paths:
            # If passed as a list of subjects that contain their paths, last element in list is number of timepoints, see load_subject in preprocessing
            ...
            A_samples = groupA.shape[0]
            B_samples = groupB.shape[0]
            A_partial = groupA - (groupA@np.linalg.pinv(vt))@vt
            B_partial = groupB - (groupB@np.linalg.pinv(vt))@vt
            A_dense_partial = compute_cov(A_partial, method=cov_method, log=log, shrink=None)
            B_dense_partial = compute_cov(B_partial, method=cov_method, log=log, shrink=None)

        A_dense_adj_partial = oas_estimator(A_dense_partial,n_samples=A_samples,shrink=shrinkage)
        B_dense_adj_partial = oas_estimator(B_dense_partial,n_samples=B_samples,shrink=shrinkage)
        del A_dense_partial, B_dense_partial

        A_eigs, A_filters = Large_FKT(A_dense_adj_partial, B_dense_adj_partial, n=n_filters_per_group, LOBPCG=True,num_simulations=1000,log=log,largest=True)
        B_eigs, B_filters = Large_FKT(B_dense_adj_partial, A_dense_adj_partial, n=n_filters_per_group, LOBPCG=True,num_simulations=1000,log=log,largest=True)
        del A_dense_adj_partial, B_dense_adj_partial
        ...
        if log:
            ...
            A_dense = compute_cov(groupA, method=cov_method, log=log, shrink=None)
            B_dense = compute_cov(groupB, method=cov_method, log=log, shrink=None)
            A_dense_haufe = oas_estimator(A_dense,n_samples=A_samples,shrink=shrinkage)
            B_dense_haufe = oas_estimator(B_dense,n_samples=B_samples,shrink=shrinkage)
