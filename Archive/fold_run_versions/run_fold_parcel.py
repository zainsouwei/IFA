...
from filters import TSSF, FKT, evaluate_filters, optimize_combined
...
def run_fold(outputfolder, fold, nPCA): #TODO remove nPCA from here
    ...
    train_groups = set(np.unique(family_ID[train_idx])) #TODO uncomment
    test_groups = set(np.unique(family_ID[test_idx])) #TODO uncomment
    intersection = train_groups & test_groups #TODO uncomment
    ...
    save_text_results(f"  Intersection of groups: {len(intersection)} (Groups: {intersection})", summary_file_path) #TODO uncomment
    ...
    # if Tangent_Class:
    #     eigs, filters_all = TSSF(train_covs, train_labels, clf_str=Tan_Class_Str, metric=metric, deconf=deconfound, con_confounder_train=train_con_confounders, cat_confounder_train=train_cat_confounders, z_score=0, haufe=False, visualize=True, output_dir=fold_results)
    # else:
    ...
    _, initial_filters, W, C = TSSF(train_covs, train_labels, clf_str=Tan_Class_Str, metric=metric, deconf=deconfound, con_confounder_train=train_con_confounders, cat_confounder_train=train_cat_confounders, z_score=0, haufe=False, visualize=True, output_dir=fold_results)
    # _, vec = eigh(np.cov(np.vstack(train_data).T))
    # comb = np.hstack((vec[:,-nPCA:],initial_filters[:,-(2*n_filters_per_group):]))
    filters_parcellated = optimize_combined(W, train_data, train_labels, alpha=0.5, beta=1.0, c=1.0, conn_mode="var_explained", spatial_mode="mean_difference", optimizer="ConjugateGradient", n_components=2,initial_point=initial_filters[:,[0,-1]])
    '''
    filters_dir = os.path.join(fold_output_dir, "Filters")
    if not os.path.exists(filters_dir):
        os.makedirs(filters_dir)
    filters_parcellated = np.load(os.path.join(filters_dir, "filters_parcellated.npy"))
    '''
    ############TODO Uncommment##############
    ...
    vt = np.load(os.path.join(migp_dir, f"vt_{nPCA}.npy"))
    ...
    # migp_dir = os.path.join(fold_output_dir, "MIGP")
    # filters_dir = os.path.join(fold_output_dir, "Filters")
    # e = np.load(os.path.join(migp_dir, "reducedsubsA.npy"))
    # reducedsubsB = np.load(os.path.join(migp_dir, "reducedsubsB.npy"))
    # reducedsubs = np.concatenate((reducedsubsA, reducedsubsB), axis=0)
    # haufe_filters_ortho = np.load(os.path.join(filters_dir, "haufe_filters_ortho.npy"))
    ...
    # This was here from when i was trying to combine the basis in a joint optimization
    # IFA_basis = filter_dual_regression(filters_parcellated, train_data, train_paths)
    # np.save(os.path.join(filters_dir, "IFA_basis.npy"), IFA_basis)
    ...
    # IFA_basis = np.load(os.path.join(ifa_dir, "IFA_basis.npy"))
    # whitened_IFA_basis = np.load(os.path.join(ifa_dir, "whitened_IFA_basis.npy"))
    # subs_data_IFA_VN = np.load(os.path.join(ifa_dir, "subs_data_IFA_VN.npy"))
    # IFA_spatial_maps = np.load(os.path.join(ifa_dir, "IFA_spatial_maps.npy"))
    # IFA_A = np.load(os.path.join(ifa_dir, "IFA_A.npy"))
    # IFA_W = np.load(os.path.join(ifa_dir, "IFA_W.npy"))
    # IFA_zmaps = np.load(os.path.join(ifa_dir, "IFA_zmaps.npy"))
    # IFA_zmaps_thresh = np.load(os.path.join(ifa_dir, "IFA_zmaps_thresh.npy"))
    # ICA_basis = np.load(os.path.join(gica_dir, "ICA_basis.npy"))
    # whitened_ICA_basis = np.load(os.path.join(gica_dir, "whitened_ICA_basis.npy"))
    # subs_data_ICA_VN = np.load(os.path.join(gica_dir, "subs_data_ICA_VN.npy"))
    # ICA_spatial_maps = np.load(os.path.join(gica_dir, "ICA_spatial_maps.npy"))
    # ICA_A = np.load(os.path.join(gica_dir, "ICA_A.npy"))
    # ICA_A = np.load(os.path.join(gica_dir, "ICA_W.npy"))
    # ICA_zmaps = np.load(os.path.join(gica_dir, "ICA_zmaps.npy"))
    # ICA_zmaps_thresh = np.load(os.path.join(gica_dir, "ICA_zmaps_thresh.npy"))
    ############TODO DELETE##############

    # Dual Regression for both basis so only need to load subject into memory once
    # dual_dir = os.path.join(fold_output_dir, "Dual_Regression") #TODO Uncomment
    ...
    IFA_results_normalized, ICA_results_normalized = evaluate_IFA_results(
        ...
        permutations=10000, correction='bonferroni', metric=metric,
